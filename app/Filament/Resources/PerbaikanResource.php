...
class PerbaikanResource extends Resource
{
    ...
    Action::make('update_status')
        ...
        ->visible(fn (Perbaikan $record): bool =>
            ...
            && $record->app_validasi !== 'divalidasi'
        )
        ->form([
            Forms\Components\Select::make('status_perbaikan')
                ->label('Status Baru')
    ...
    if ($record->status_perbaikan === 'selesai' || $record->app_validasi === 'divalidasi') {
        Notification::make()
            ->title('Perbaikan sudah selesai atau sudah divalidasi.')
            ->body('Status ini tidak bisa diubah lagi oleh admin/asisten lab.')
            ->warning()
            ->send();
    ...
    // Saat status diubah menjadi dikerjakan dari Kelola Perbaikan,
    // tanggal mulai langsung tersimpan.
    ...
    Action::make('selesaikan')
        ...
        ->action(function (Perbaikan $record, array $data): void {
            ...
            if ($record->status_perbaikan === 'selesai' || $record->app_validasi === 'divalidasi') {
                Notification::make()
                    ->title('Perbaikan sudah selesai atau sudah divalidasi.')
                    ->body('Data ini tidak bisa diselesaikan ulang.')
                    ->warning()
                    ->send();
    ...
    Action::make('validasi_spv')
        ...
        ->action(function (Perbaikan $record): void {
            ...
            if ($record->status_perbaikan !== 'selesai' || $record->app_validasi !== 'menunggu') {
                Notification::make()
                    ->title('Perbaikan tidak bisa divalidasi.')
                    ->warning()
                    ->send();
    ...
    Action::make('kembalikan_spv')
        ...
        ->action(function (Perbaikan $record, array $data): void {
            ...
            if ($record->status_perbaikan !== 'selesai' || $record->app_validasi !== 'menunggu') {
                Notification::make()
                    ->title('Perbaikan tidak bisa dikembalikan.')
                    ->warning()
                    ->send();
